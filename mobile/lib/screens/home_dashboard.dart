  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: ListView(padding: const EdgeInsets.all(22), children: [
        Row(children: [
          CircleAvatar(radius: 24, backgroundColor: RColors.lavender.withValues(alpha: .25), child: const Icon(Icons.person, color: RColors.lavenderDeep)),
        ...
        const SizedBox(height: 18),
        GridView.count(crossAxisCount: 2, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), mainAxisSpacing: 14, crossAxisSpacing: 14, childAspectRatio: 1.35, children: cards.map((c) => _QuickCard(icon: c.$1, title: c.$2, sub: c.$3)).toList()),
      ])),
    );
  }
}
...
class _QuickCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white.withValues(alpha: .7), borderRadius: BorderRadius.circular(22), boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: .04), blurRadius: 12, offset: const Offset(0, 4))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(width: 34, height: 34, decoration: BoxDecoration(color: RColors.lavender.withValues(alpha: .18), borderRadius: BorderRadius.circular(10)), child: Icon(icon, size: 18, color: RColors.lavenderDeep)),
        const Spacer(),
        Text(title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
        Text(sub, style: TextStyle(fontSize: 11, color: RColors.inkSoft)),
      ]),
    );
  }
}
